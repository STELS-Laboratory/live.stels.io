...
export function extractConnectionKeys(
  ...
  if (
    nodeData.isSchema && nodeData.channelKeys && nodeData.channelKeys.length > 0
  ) {
  ...
  if (nodeData.sessionData) {
    ...
    if (raw?.exchange && typeof raw.exchange === "string") {
      ...
      if (exchangeMatch) {
        ...
        const fallbackMatch = sessionData.module.match(
          /(binance|bybit|coinbase|kraken|okx|kucoin)/i,
        );
        if (fallbackMatch) {
 ...
 */
export function extractSmartConnectionKeys(
  nodeData: FlowNodeData,
): ConnectionKeys {
...
export function analyzeChannelPatterns(nodes: FlowNode[]): {
  ...
  nodes.forEach((node) => {
    ...
    else if (channel.includes("connector.exchange")) {
      patternType = "connector.exchange";
