...
import type { MessageBatch, PublicWebSocketConfig } from "@/types/hooks/types";
        ...
        requestAnimationFrame(() => {
          try {
            ...
            if (json.value) {
              ...
              publicMessageBatch[json.value.channel] = JSON.stringify(
                json.value,
              );
      ...
      ws.onclose = (event: CloseEvent): void => {
        ...
        if (reconnectAttemptsRef.current < maxReconnectAttempts) {
          ...
          const delay = Math.min(
            1500 * Math.pow(2, reconnectAttemptsRef.current - 1),
            30000,
          );
