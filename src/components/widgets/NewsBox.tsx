...
function NewsBox(
	...
	return (
		<div
			className={`w-[1044px] bg-gradient-to-br ${getTypeColor()} backdrop-blur-sm border shadow-lg p-6`}
		>
			{/* Header */}
			<div className="flex items-center justify-between mb-6">
				<div className="flex items-center gap-3">
					<div className="w-3 h-3 rounded-full bg-muted-foreground dark:bg-muted shadow-sm" />
					<h2 className="text-xl font-medium text-foreground uppercase tracking-wide">
						{type} News
					</h2>
				</div>
				<div className="text-sm text-muted-foreground font-mono">
					{data?.raw?.length || 0} articles
				</div>
			</div>
					...
					return (
						...
						>
							...
							<div className="absolute top-2 right-2">
								<div
									className={`px-2 py-1 rounded-full text-xs font-medium uppercase tracking-wide ${styles.ratingBg} ${styles.ratingText} border ${
							...
							<div className="flex items-start justify-between gap-4 mb-3 pr-16">
								<div className="flex-1">
									...
									<div className="flex items-center gap-4 text-sm">
										<span className="text-muted-foreground font-medium">
											{extractSource(article.description)}
										</span>
										<span className="text-muted-foreground">•</span>
										<span className="text-muted-foreground font-mono">
											{formatTimeAgo(article.ageMs)}
										</span>
									</div>
								</div>
								...
								>
									...
									<div
										className={`text-2xl font-bold ${styles.ratingText} font-mono leading-none`}
									>
										{article.score}
									</div>
										...
										{[...Array(5)].map((_, i) => (
											<div
												...
												className={`w-1.5 h-1.5 rounded-full ${
													i < Math.floor(article.score / 20)
														? styles.indicator
														: "bg-muted-foreground dark:bg-zinc-600"
												}`}
											/>
										))}
									</div>
