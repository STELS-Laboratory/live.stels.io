...
import { parseTradingPair } from "@/lib/utils";
...
export default function OrderBook({ book }: { book: OrderBookData }) {
	...
	useEffect(() => {
		...
		book.bids.forEach((bid, index) => {
			...
			const isNew = !prevBid;
			const isUpdated = prevBid &&
				(prevBid[0] !== bid[0] || prevBid[1] !== bid[1]);
		...
		book.asks.forEach((ask, index) => {
			...
			const isNew = !prevAsk;
			const isUpdated = prevAsk &&
				(prevAsk[0] !== ask[0] || prevAsk[1] !== ask[1]);
	...
	useEffect(() => {
		...
		// VWAP calculation
		const bidVwap = book.bids.reduce((sum, [price, vol]) =>
			sum + price * vol, 0) / bidVolume;
		const askVwap = book.asks.reduce((sum, [price, vol]) =>
			sum + price * vol, 0) / askVolume;
		...
		const prevAskPrice = prevBook.asks.length > 9
			? (prevBook.asks.length > 9 ? prevBook.asks[9][0] : 0)
			: 0;
		const prevMidPrice = (prevBidPrice + prevAskPrice) / 2;
		const timeDiff = (book.timestamp - prevBook.timestamp) / 1000; // in seconds
		const priceVelocity = timeDiff > 0
			? (midPrice - prevMidPrice) / timeDiff
			: 0;
	...
	const getSpreadInfo = (
		...
		const currentOrderIndex = sortedOrders.findIndex((order) =>
		...
		if (currentOrderIndex < sortedOrders.length - 1) {
			...
			const spreadValue = nextOrder.price -
				sortedOrders[currentOrderIndex].price;
		...
		if (currentOrderIndex > 0) {
			...
			const spreadValue = sortedOrders[currentOrderIndex].price -
				prevOrder.price;
			return `${spreadValue.toFixed(1)}`;
		}
	...
	return (
		<motion.div
			...
			className="w-[520px] bg-gradient-to-b from-card to-background rounded-xl border border-border/50 shadow-xl overflow-hidden backdrop-blur-sm"
		>
			<header className="border-b border-border/50 p-4 bg-background/40">
				<div className="flex justify-between items-center">
					<div>
						<h2 className="text-sm flex items-center gap-2">
							...
							<Badge
								variant="outline"
								className="text-[10px] font-normal h-5 px-2 bg-card/80 border-border"
							>
								{book.exchange}
							</Badge>
						</h2>
					...
					<div className="text-right text-[10px]">
						...
						<div className="font-mono text-card-foreground mt-1">
							{book.volume[0].toFixed(2)} {base} / {book.volume[1].toFixed(2)}
							{" "}
							{base}
						</div>
					</div>
			...
			{/* Technical Metrics */}
			<div className="grid grid-cols-3 gap-2 border-b border-border/50 p-4 bg-card/30">
				<TooltipProvider>
					<Tooltip>
						<TooltipTrigger asChild>
							<motion.div
								whileHover={{ scale: 1.01 }}
								className="bg-muted/30 dark:bg-black/30 p-2 rounded border border-border/50 cursor-help"
							>
							...
							>
								...
								<div className="font-mono flex items-center mt-1">
									{metrics.priceVelocity > 0
										...
										: <TrendingDown className="h-3 w-3 text-red-500 mr-1" />}
									<span
										className={metrics.priceVelocity > 0
											? "text-green-500"
											: "text-red-500"}
									>
				...
				<div className="bg-muted/30 dark:bg-black/30 p-2 rounded border border-border/50 flex flex-col justify-between">
					<div className="text-[10px] text-muted-foreground uppercase tracking-wider">
						CONTROLS
					</div>
					<div className="flex items-center justify-between gap-2">
						<button
							onClick={() =>
								setViewMode(viewMode === "standard" ? "depth" : "standard")}
							className="text-muted-foreground hover:text-amber-500 transition-colors"
							title={viewMode === "standard"
			...
			{/* Column Headers */}
			<div className="grid grid-cols-6 text-[10px] text-muted-foreground uppercase tracking-wider py-2 px-4 bg-card/30">
			...
			>
				{Array.from({ length: Math.max(book.bids.length, book.asks.length) })
					.map((_unused, index) => {
						...
						const bidValue = index < book.bids.length
							? book.bids[index][0] * book.bids[index][1]
							: 0;
						const bidSpread = isLargeBid
							? getSpreadInfo(largeBids, index)
							: null;

						// Find if this ask is a large order
						const isLargeAsk = index < book.asks.length &&
							book.asks[index][1] > largeOrderThreshold;
						const askValue = index < book.asks.length
							? book.asks[index][0] * book.asks[index][1]
							: 0;
						const askSpread = isLargeAsk
							? getSpreadInfo(largeAsks, index)
							: null;

						// Get cumulative volumes for scales
						const bidCumulative = index < cumulativeVolume.bids.length
							? cumulativeVolume.bids[index].cumulative
							: 0;
						const askCumulative = index < cumulativeVolume.asks.length
							? cumulativeVolume.asks[index].cumulative
							: 0;
						...
						return (
							...
							>
								...
								{index < book.bids.length && (
									<>
										<div className="relative font-mono text-muted-foreground text-[9px] text-left pl-2 flex items-center z-10">
											{book.bids[index][1].toFixed(3)}
											{showScales && (
												<span className="ml-1 text-[7px] text-green-700">
													Σ{formatVolume(bidCumulative)}
												</span>
											)}
										</div>
										{isLargeBid
											? (
												<div className="col-span-2 flex items-center justify-end z-10">
													<motion.div
														className="bg-card/80 border border-green-800/50 rounded-md px-2 mr-2 h-5 flex items-center"
														whileHover={{ scale: 1.05 }}
													>
														<span className="text-[7px] font-semibold text-pink-600 mr-1">
															W
														</span>
														...
														{bidSpread && (
															<span className="text-[7px] text-green-500 ml-1">
																Δ{bidSpread}
															</span>
														)}
													</motion.div>
										...
										{showScales && (
											<motion.div
												...
												style={{
													width: `${
														(bidCumulative / cumulativeVolume.maxCumulative) *
														50
													}%`,
												}}
												initial={{ width: 0 }}
											...
											{bidAnimation && (
												<motion.div
													...
													className={`absolute left-0 h-6 ${
														getAnimationColor("bid", bidAnimation.isNew)
													} rounded-r-sm opacity-10`}
								...
								{index < book.asks.length && (
									<>
										{isLargeAsk
											? (
												<div className="col-span-2 flex items-center z-10">
													...
													<motion.div
														className="bg-card/80 border border-red-800/50 rounded-md px-2 ml-2 h-5 flex items-center"
														whileHover={{ scale: 1.05 }}
													>
														<span className="text-[7px] font-semibold text-lime-600 mr-1">
															W
														</span>
														<span className="text-[7px] text-red-400">
															{formatCurrency(askValue)}
														</span>
														{askSpread && (
															<span className="text-[7px] text-red-500 ml-1">
																Δ{askSpread}
															</span>
														)}
													</motion.div>
										...
										<div className="relative font-mono text-muted-foreground text-[9px] text-right pr-2 flex items-center justify-end z-10">
											{showScales && (
												<span className="mr-1 text-[7px] text-red-700">
													Σ{formatVolume(askCumulative)}
												</span>
											)}
										...
										{showScales && (
											<motion.div
												...
												style={{
													width: `${
														(askCumulative / cumulativeVolume.maxCumulative) *
														50
													}%`,
												}}
												initial={{ width: 0 }}
											...
											{askAnimation && (
												<motion.div
													...
													className={`absolute right-0 h-6 ${
														getAnimationColor("ask", askAnimation.isNew)
													} rounded-l-sm opacity-10`}
													initial={{ width: 0, opacity: 0.2 }}
													animate={{ width: "100%", opacity: 0 }}
													exit={{ opacity: 0 }}
													transition={{ duration: 0.6 }}
				...
				<div className="flex flex-col items-center">
					...
					<div className="w-full flex items-center justify-between mb-2">
						<div className="text-[10px] text-red-500 font-semibold">
							SELLERS {dominance.sellers.toFixed(1)}%
						</div>
						<div className="text-[10px] text-green-500 font-semibold">
							{dominance.buyers.toFixed(1)}% BUYERS
						</div>
					</div>
					<div className="w-full h-6 bg-card/80 rounded-full overflow-hidden relative border border-border/50">
						{/* Balance indicator */}
						<div className="absolute inset-0 flex">
							...
							>
								{dominance.sellers >= 15 && (
									<span className="text-[10px] font-mono text-foreground px-1">
										{dominance.sellers.toFixed(1)}%
									</span>
								)}
							</motion.div>
							<motion.div
