...
function normalizeCallbackUrl(backendUrl: string): string {
	...
	if (url.hostname === '10.0.0.206' || url.hostname === '127.0.0.1' || url.hostname === '0.0.0.0') {
...
function getGitHubOAuthUrl(backendUrl: string): string {
	...
	if (!clientId) {
		throw new Error("GitHub Client ID is not configured. Please set VITE_GITHUB_CLIENT_ID environment variable.");
	}
...
export function GitHubAuth({
	...
	const { selectedNetwork, connectWithGitHub, isConnecting, connectionError } = useAuthStore();
	...
	const handleGitHubLogin = useCallback((): void => {
		...
		try {
			...
			const errorMessage = err instanceof Error ? err.message : 'Failed to initiate GitHub login';
			setError(errorMessage);
			setIsRedirecting(false);
			if (onError) {
				onError();
			...
			<CardContent className="space-y-4">
							...
							{displayError.includes('VITE_GITHUB_CLIENT_ID') && (
								<div className="text-xs mt-2 space-y-1">
									...
									<ol className="list-decimal list-inside space-y-1 ml-2">
										<li>Create a <code className="bg-muted px-1 py-0.5 rounded text-xs">.env</code> file in the project root</li>
										<li>Add: <code className="bg-muted px-1 py-0.5 rounded text-xs">VITE_GITHUB_CLIENT_ID=your_client_id_here</code></li>
				...
				{selectedNetwork && (
					<div className="text-sm text-muted-foreground text-center">
						Connecting to: <span className="font-semibold text-foreground">{selectedNetwork.name}</span>
					</div>
				)}

				<Button
					onClick={handleGitHubLogin}
					disabled={isConnecting || isRedirecting || !selectedNetwork || !isClientIdConfigured}
				...
				{!isClientIdConfigured && !displayError && (
					<Alert>
						<AlertCircle className="h-4 w-4" />
						<AlertDescription className="space-y-2">
							<div className="font-semibold">GitHub Client ID is not configured</div>
							<div className="text-xs space-y-1">
								...
								<ol className="list-decimal list-inside space-y-1 ml-2">
									...
									<li>Add: <code className="bg-muted px-1 py-0.5 rounded">VITE_GITHUB_CLIENT_ID=your_client_id</code></li>
									<li>Restart the development server</li>
								</ol>
								<p className="mt-2 font-semibold">Important: GitHub OAuth App Configuration</p>
								<p className="text-xs">
									In your GitHub OAuth App settings, set:
									<br />
									<code className="bg-muted px-1 py-0.5 rounded">Authorization callback URL: http://localhost:8088/auth/github/callback</code>
									<br />
									<small className="text-muted-foreground">
										(For local development, use localhost even if backend uses IP address)
									</small>
									<br />
									<small className="text-muted-foreground">
										Or set <code className="bg-muted px-1 py-0.5 rounded">VITE_GITHUB_CALLBACK_URL</code> in .env to override
									</small>
								</p>
								<p className="mt-2">
									Get your Client ID from{' '}
