...
export function TokenListItem({
  ...
  const formattedMaxSupply = supply?.max
    ...
    : supply?.max === undefined
    ? "Unlimited"
    : "N/A";
  const hasDistribution = token.economics?.distribution &&
    token.economics.distribution.length > 0;
      ...
      <div className="text-right flex-shrink-0 min-w-[100px] hidden lg:block">
        ...
        {hasDistribution && (
          <div className="text-xs text-muted-foreground">
            {token.economics.distribution?.length}{" "}
            holder{token.economics.distribution?.length === 1 ? "" : "s"}
          </div>
        )}
      </div>
