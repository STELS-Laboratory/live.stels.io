  ...
  try {
    ...
    switch (message.type) {
      case "parse": {
        ...
        if (
          parserType === "orderBook" ||
          (parserType === "auto" &&
            (message.data as { raw?: { bids?: unknown; asks?: unknown } }).raw
              ?.bids)
        ) {
          ...
          parserType === "ticker" ||
          (parserType === "auto" &&
            (message.data as { raw?: { last?: unknown } }).raw?.last !==
              undefined)
        ) {
          result = parseTicker(message.data, symbol);
        } else if (
          parserType === "accountBalance" ||
          (parserType === "auto" &&
            ((message.data as { raw?: { BTC?: unknown; ETH?: unknown } }).raw
              ?.BTC ||
              (message.data as { raw?: { wallet?: unknown } }).raw?.wallet))
        ) {
          result = parseAccountBalance(message.data);
        } else if (
          parserType === "candles" ||
          (parserType === "auto" &&
            (message.data as { raw?: { candles?: unknown } }).raw?.candles)
        ) {
          result = parseCandles(message.data);
        } else if (
          parserType === "index" ||
          (parserType === "auto" &&
            (message.data as { raw?: { index?: unknown } }).raw?.index)
        ) {
      ...
      case "parseBatch": {
        ...
        if (message.channels && message.dataArray) {
          for (
            let i = 0;
            i < message.channels.length && i < message.dataArray.length;
            i++
          ) {
            ...
            if (
              (data as { raw?: { bids?: unknown; asks?: unknown } }).raw?.bids
            ) {
              results[channel] = parseOrderBook(data);
            } else if (
              (data as { raw?: { last?: unknown } }).raw?.last !== undefined
            ) {
              results[channel] = parseTicker(data, "");
            } else if (
              (data as { raw?: { BTC?: unknown; ETH?: unknown } }).raw?.BTC ||
              (data as { raw?: { wallet?: unknown } }).raw?.wallet
            ) {
