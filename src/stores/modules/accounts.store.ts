	...
	devtools(
		persist(
			(set, get) => ({
				...
				updateAccount: (id: string, updates: Partial<AccountRequest>): boolean => {
					const { accounts } = get();
				...
				sendAccountToServer: async (account: AccountRequest, session: string, apiUrl: string): Promise<boolean> => {
