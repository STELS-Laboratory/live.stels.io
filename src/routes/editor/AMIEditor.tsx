...
export function AMIEditor(): JSX.Element {
	...
	const listWorkers = useAppStore((state) => state.listWorkers);
	...
	const setWorker = useAppStore((state) => state.setWorker);
	...
	const loadWorkers = async () => {
		...
		try {
			await listWorkers();
			const w = useAppStore.getState().workers;
			setWorkers(w);
			setLoading(false);
		} catch (error) {
	...
	// Clear newly created worker highlight after 3 seconds
	useEffect(() => {
