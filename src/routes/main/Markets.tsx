...
function MiniCandlestickChart({ candles, width = 120, height = 40 }: MiniCandlestickChartProps) {
	...
	return (
		<svg width={width} height={height} className="overflow-visible">
			{recentCandles.map((candle, index) => {
				...
				const highY = 5 + ((maxPrice - candle.high) / priceRange) * (height - 10)
				const lowY = 5 + ((maxPrice - candle.low) / priceRange) * (height - 10)
				const openY = 5 + ((maxPrice - candle.open) / priceRange) * (height - 10)
				const closeY = 5 + ((maxPrice - candle.close) / priceRange) * (height - 10)
				...
				return (
					<g key={index}>
						{/* Wick */}
						<line x1={x} y1={highY} x2={x} y2={lowY} stroke={isGreen ? "#16a34a" : "#dc2626"} strokeWidth="0.5" />
						{/* Body */}
						<rect
							x={x - candleWidth / 4}
							y={bodyTop}
...
function Markets() {
	...
	const formattedTickers = useMemo(() => {
		return spotTickers.map((ticker: any) => {
			...
			const candleData = spotCandles.find((candle: any) => candle.value.raw.market === raw.market)
			
			const candles = candleData
				? candleData.value.raw.candles.map((c: any[]) => ({
					timestamp: c[0],
					open: c[1],
					high: c[2],
					low: c[3],
	...
	return (
		<Screen>
			<Header title="Markets" description="LIVE Markets list"/>
			
			<Card>
				<CardHeader>
					<CardTitle>Live Ticker Data</CardTitle>
					<CardDescription>Real-time cryptocurrency futures prices and market data</CardDescription>
				</CardHeader>
				<CardContent>
					<div className="rounded-md border">
						<Table>
							...
							<TableBody>
								{formattedTickers.map((ticker: any) => (
									<TableRow key={ticker.market}>
										<TableCell className="font-medium">
											<div className="flex items-center gap-2">
												...
												<div>
													...
													<div className="text-xs text-muted-foreground">{ticker.market}</div>
												</div>
											</div>
										</TableCell>
										<TableCell>
										...
										<TableCell className="text-right font-mono">{formatPrice(ticker.price, ticker.symbol)}</TableCell>
										...
										<TableCell className="text-right">
											<div className="space-y-1">
												<div className="font-mono text-sm">{formatVolume(ticker.volume)}</div>
												<div className="text-xs text-muted-foreground">${formatVolume(ticker.quoteVolume)}</div>
											</div>
										</TableCell>
										<TableCell className="text-right font-mono text-xs">
											<div className="space-y-1">
												<div className="text-green-600">{formatPrice(ticker.bid, ticker.symbol)}</div>
												<div className="text-red-600">{formatPrice(ticker.ask, ticker.symbol)}</div>
											</div>
										</TableCell>
										<TableCell className="text-right">
											<Badge variant={ticker.latency < 3000 ? "default" : "destructive"} className="text-xs">
												{ticker.latency}ms
											</Badge>
										</TableCell>
									</TableRow>
