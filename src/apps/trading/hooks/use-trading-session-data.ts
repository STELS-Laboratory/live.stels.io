...
function parseAccountBalance(
  ...
  try {
    ...
    if (
      raw.wallet?.info?.result?.list &&
      Array.isArray(raw.wallet.info.result.list) &&
      raw.wallet.info.result.list.length > 0
    ) {
      ...
      if (Array.isArray(coins)) {
        coins.forEach((coin) => {
          ...
          const available = Number(coin.availableToWithdraw) ||
            (walletBalance > locked ? walletBalance - locked : walletBalance);

          // Always use wallet format data as it's more accurate
          balances[currency] = {
            free: available > 0
              ? available
              : (equity > 0 ? equity : walletBalance),
            used: locked,
    ...
    directCurrencyKeys.forEach((key) => {
      ...
      if (!balances[key]) {
        ...
        if (
          balance &&
          (balance.free !== undefined || balance.used !== undefined ||
