...
export class TradingApiService {
  ...
  private transformOrder(rawOrder: Record<string, unknown>): TradingOrder {
        ...
        if (typeof info === "object" && info !== null) {
          for (const infoKey in info) {
            ...
            if (nested && typeof nested === "object") {
              const nestedValue = (nested as Record<string, unknown>)[key] as
                | T
                | undefined;
    ...
    if (isExplicitlyCanceled || hasValidCancelType) {
      ...
      switch (rawStatus.toLowerCase()) {
        ...
        default:
          ...
          if (
            !["open", "closed", "canceled", "filled", "partiallyFilled"]
              .includes(status)
          ) {
            status = "unknown";
          }
    ...
    const remaining = getValue<number>(
      ...
      "remainingQty",
      "RemainingQty",
    ) ?? (amount ? amount - filled : undefined);
    const cost = getValue<number>(
      ...
      "executedValue",
      "ExecutedValue",
    ) ?? (price && filled ? price * filled : undefined);
    ...
    const datetime = (rawOrder.datetime as string) ||
      (info.datetime as string) || new Date(timestamp).toISOString();

    // Order ID handling
    const id = (rawOrder.id as string) || (info.orderId as string) ||
      (info.OrderId as string) || "";
    const clientOrderId = (rawOrder.clientOrderId as string) ||
      (info.orderLinkId as string) || (info.OrderLinkId as string) ||
      (info.clientOrderId as string) || undefined;
  ...
  async listTrades(params: {
    ...
    const result = await this.makeRequest<
      { trades: TradingTrade[]; total: number }
    >(
      "listTrades",
      params,
    );
