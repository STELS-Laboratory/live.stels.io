...
import type { Panel, PanelData } from "@/lib/panel-types.ts";
import type { GroupingMode } from "@/lib/canvas-types.ts";
import type { CanvasStore } from "@/types/apps/canvas/types";
    ...
    persist(
      (set, get) => ({
        ...
        deletePanel: (panelId: string) => {
          set((state) => {
            ...
            if (state.panels.activePanelId === panelId) {
              newActivePanelId = remainingPanels.length > 0
        ...
        getActivePanel: (): Panel | null => {
          const state = get();
          return (
            state.panels.panels.find((p) =>
              p.id === state.panels.activePanelId
            ) ||
            null
          );
        },
          ...
          try {
            ...
            set({
              panels: {
                ...
                activePanelId: importData.panels.length > 0
                  ? importData.panels[0].id
                  : null,
