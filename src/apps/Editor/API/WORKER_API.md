...
**⚠️ Ограничения по изменению полей:**

- 🔒 **Поле `scope` нельзя изменить** через `updateWorker` после создания
  воркера
- 🔒 **Поле `sid` нельзя изменить** (генерируется один раз при создании)
- Для миграции `scope` используйте:
  - `migrateWorker` — сохраняет существующий `sid`, атомарная операция
  - `setWorker` (новый) — генерирует новый `sid`, безопасная миграция
- При попытке изменить `executionMode` на недопустимый для текущего `scope`
  будет возвращена ошибка
- Валидация: `scope: "local"` + `executionMode: "parallel"/"exclusive"` = ошибка
  400
...
- **⚠️ ВАЖНО:** Worker ID (`sid`) **сохраняется** при миграции через
  `migrateWorker`
- Если воркер уже в целевом `scope`, операция возвращает успех без изменений
- ⚠️ **Это единственный способ изменить `scope` существующего воркера** (через
  `updateWorker` нельзя)
...
- При миграции с `local` → `network`: воркер становится доступен всем узлам в
  сети
- При миграции `local` → `network` автоматически проверяется совместимость
  `executionMode`
- Если воркер активен, он будет перезапущен после миграции

**Выбор метода миграции:**

- 🔄 **`migrateWorker`** — сохраняет UUID, атомарная операция, изменяет
  существующий воркер
- 🆕 **`setWorker` (новый)** — создает новый UUID, безопасная миграция, старый
  воркер остается нетронутым
...
- ✨ **BREAKING:** Локальные воркеры теперь поддерживают только
  `executionMode: "leader"`
- ✨ **BREAKING:** Поле `scope` нельзя изменить через `updateWorker` после
  создания воркера
...
- ✨ `migrateWorker` — единственный способ изменить `scope` с сохранением `sid`
...
- 🔥 UI: поле Scope read-only в конфигурации существующих воркеров
...
- 🔒 **Scope read-only** → нельзя изменить через `updateWorker` после создания
- 🔄 **Миграция `migrateWorker`** → изменяет scope с сохранением UUID
- 🆕 **Миграция `setWorker`** → создает новый воркер с новым UUID (безопасно)
