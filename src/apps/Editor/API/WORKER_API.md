...
Worker API предоставляет управление распределенными воркерами в Heterogen Runtime. Воркеры представляют собой JavaScript скрипты, выполняемые на узлах сети с поддержкой двух уровней видимости:

### Scope (Область видимости)

- **local** — воркер хранится в локальной KV базе данных, виден только на текущем узле (по умолчанию)
- **network** — воркер хранится в распределенной KV базе данных, виден всем узлам в сети

### Execution Mode (Режим выполнения)

- **parallel** — выполняется на всех узлах одновременно
- **leader** — выполняется только на одном узле-лидере (distributed consensus) ⚠️ не применимо для `scope: "local"`
- **exclusive** — выполняется только на назначенном узле

### Конфигурация WORKERS

Переменная окружения `WORKERS` контролирует обработку **только сетевых воркеров** (`scope: "network"`):
...
- **`scope: "local"`** (по умолчанию) — воркер виден только на текущем узле, не реплицируется в сеть
- **`scope: "network"`** — воркер виден всем узлам в сети
- ⚠️ **Для `scope: "local"` режим `executionMode: "leader"` не применим** (будет работать как `parallel`)
...
- **При изменении `scope`** воркер автоматически мигрирует между локальным и сетевым хранилищем
...
- Миграция выполняется атомарно: создается в новом месте, затем удаляется из старого
...
- **Альтернатива**: можно использовать `updateWorker` с изменением `scope` для той же цели
- При миграции с `network` → `local`: воркер перестает быть виден другим узлам
- При миграции с `local` → `network`: воркер становится доступен всем узлам в сети
  ...
  body: JSON.stringify({
    ...
    body: {
      scriptContent: `
        ...
        // Используйте shouldStop() для корректной остановки
        while (!shouldStop()) {
          logger.info('Working...');
...
await fetch("http://localhost:3000", {
  ...
  body: JSON.stringify({
    ...
    body: {
      ...
      `,
      scope: "network", // Leader режим требует network scope
      executionMode: "leader",
      priority: "high",
    },
  }),
...
// Для длительных операций проверяйте shouldStop()
while (!shouldStop()) {
  await doWork();
...
// НЕ используйте бесконечные циклы без проверки
while (true) { // Воркер не остановится!
  await doWork();
}
```
...
- **`scope: "local"`** (по умолчанию) — для разработки, тестирования и задач, специфичных для узла:
...
- **`executionMode: "parallel"`** (работает с `local` и `network`):
  - Задачи на каждом узле
  - Мониторинг локальных ресурсов
  - Независимые операции

- **`executionMode: "leader"`** (требует `scope: "network"`):
  - Глобальные задачи (один раз на всю сеть)
  - Агрегация данных
  - Рассылка уведомлений
  - Distributed consensus

- **`executionMode: "exclusive"`** (работает с `local` и `network`):
  - Задачи для конкретного узла
...
- ✨ Добавлена поддержка `scope: "local" | "network"` для контроля видимости воркеров
- ✨ **По умолчанию воркеры создаются локально** (`scope: "local"`) и выполняются только на текущем узле
- ✨ **Локальные воркеры работают всегда**, даже при `WORKERS=0` в конфигурации
- ✨ **Локальные воркеры автоматически являются лидерами** - не нужен leader election
- ✨ Параметр `WORKERS` теперь контролирует только сетевые воркеры (`scope: "network"`)
- ✨ Добавлен endpoint `migrateWorker` для перемещения воркеров между local и network
- ✨ `updateWorker` теперь поддерживает автоматическую миграцию при изменении `scope`
- ✨ `listWorkers` возвращает воркеры из обоих источников (local + network)
- ✨ `stopAllWorkers` останавливает воркеры в обоих хранилищах
- ✨ Добавлена функция `shouldStop()` для корректной остановки воркеров с циклами
- ✨ Добавлен доступ к `signal` для продвинутого контроля воркеров
- 🔥 Worker Engine читает и запускает воркеры из обоих источников
- 🔥 Улучшена обработка остановки воркеров
- 🐛 **Исправлено накопление воркеров при hot reload** - теперь старый воркер полностью останавливается перед запуском нового
- 🐛 **Исправлена принудительная остановка** - `Stels.sleep()` теперь прерывается мгновенно при abort signal
...
**Документация актуальна для:** Heterogen Runtime v2.1.0
